...
import { getProjects, createProject, updateProject, deleteProject, getBoards, createBoard, updateBoard, deleteBoard, getTasks, createTask, updateTask, deleteTask, getSubtasks, createSubtask, updateSubtask, deleteSubtask, } from '../api';
...
export function ProjectsPage() {
    ...
    const [subtasksMap, setSubtasksMap] = useState({}); // taskId -> subtasks
    ...
    useEffect(() => {
        (async () => {
            ...
            setTasks(all);
            // 并行加载每个任务的子任务
            const pairs = await Promise.all(all.map(async (t) => [t.id, await getSubtasks(t.id)]));
            const map = {};
            for (const [id, subs] of pairs)
                map[id] = subs;
            setSubtasksMap(map);
        })();
    }, [activeProject]);
    ...
    async function renameSubtask(taskId, sub) {
        const title = prompt('新标题', sub.title)?.trim();
        if (!title)
            return;
        const updated = await updateSubtask(sub.id, { title });
        setSubtasksMap(prev => ({ ...prev, [taskId]: (prev[taskId] ?? []).map(s => s.id === sub.id ? updated : s) }));
    }
    async function removeSubtask(taskId, sub) {
        if (!confirm('删除子任务？'))
            return;
